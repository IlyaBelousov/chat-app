...
export const useTheme = () => {
  ...
  const toggleTheme = async () => {
    const prevTheme = ((await AsyncStorage.getItem(LocalStorageKeys.theme)) as ThemeKeys)
      || ThemeKeys.dark;

    const correctedTheme = prevTheme === ThemeKeys.dark ? ThemeKeys.light : ThemeKeys.dark;
